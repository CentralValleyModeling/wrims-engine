...
public class WeightEval {
    ...
    private static final String fstr = "WeightTable.csv";
    private static final String farstr = "WeightTableAR.csv";

    private static final ArrayList<String> wList = new ArrayList<String>();
    private static final HashMap<String, LinkedHashMap<Integer, ArrayList<Double>>> wMap = new HashMap<String, LinkedHashMap<Integer, ArrayList<Double>>>();
    private static final ArrayList<String> variedwList = new ArrayList<String>();

    private static final CopyOnWriteArrayList<String> wListAR = new CopyOnWriteArrayList<String>();
    private static final ConcurrentHashMap<String, WeightElement> wMapAR = new ConcurrentHashMap<String, WeightElement>();
    ...
    public static void outputExtreme() {
        ...
        if (size > 0) {
            ...
            LOGGER.atInfo()
                  .setMessage(
                          "have variable(s) in the weight and were excluded from the evaluation of the maximum and minimum weights.")
                  .log();
        }
        for (int i = 0; i < maxwva.size(); i++) {
            LOGGER.atInfo()
                  .setMessage("The maximum weight is " + maxwva.get(i) + " with a value of " + maxw + " and appear in Cycle " + maxCa.get(
                          i) + ".")
                  .log();
            LOGGER.atInfo().setMessage("The maximum weight source code location is at " + maxPa.get(i) + " .").log();
        }
        for (int i = 0; i < minwva.size(); i++) {
            LOGGER.atInfo()
                  .setMessage("The minimum weight is " + minwva.get(i) + " with a value of " + minw + " and appear in Cycle " + minCa.get(
                          i) + ".")
                  .log();
            LOGGER.atInfo().setMessage("The minimum weight source code location is at " + minPa.get(i) + " .").log();
        }
    }
