...
import gov.ca.water.wrims.engine.core.commondata.wresldata.Goal;
...
import gov.ca.water.wrims.engine.core.wreslplus.elements.VarCycleIndex;
...
public class StudyParser {
    ...
    private static final int n_buffer = 3;
        ...
        for (String modelName : studyDataSet.getModelList()) {
            ...
            for (String timeseriesName : ds.tsMap.keySet()) {
                if (timeseriesTimeStepMap.containsKey(timeseriesName)) {
                    ...
                    timeseriesTimeStepMap.put(timeseriesName, new ArrayList<String>(Collections.singletonList(
                            definedTimeStep)));
                }
            }
        }
    ...
    public static StudyConfig processMainFileIntoStudyConfig(String relativeMainFilePath) throws
    ...
    public static StudyConfig processMainFileIntoStudyConfig(String relativeMainFilePath, boolean showTree) throws
            RecognitionException,
            IOException {
        ...
        for (Integer i : sc.sequenceMap.keySet()) {
            sc.sequenceOrder.add(i);
        }
        ...
        for (Integer i : sc.sequenceOrder) {
            LogUtils.importantMsg("Sequence: " + i + " : " + sc.sequenceMap.get(i).sequenceName + "   Model: " + sc.sequenceMap.get(
                    i).modelName);
        }
    ...
    public static Map<String, SimulationDataSet> parseModels(StudyConfig sc, TempData td) throws
            RecognitionException,
            IOException {
        return parseModels(sc, td, false, false);
    }

    public static Map<String, SimulationDataSet> parseModels(
            StudyConfig sc,
            TempData td,
            boolean rewrite_list_based_on_dependency,
            boolean send_all_alias_to_dvar
        ...
        for (Integer iSequence : sc.sequenceOrder) {
            ...
            Map<String, SimulationDataSet> fileDataMap_new = getNewDataSet(
                    adhoc.incFileSet,
                    td.fileDataMap_wholeStudy.keySet()
            );
            ...
            Map<String, SimulationDataSet> fileDataMap_thisModel = copyDataSetToThisModel(
                    adhoc.incFileSet,
            ...
            SimulationDataSet model_dataset = correctScopeAndPrioritize(
                    modelName,
                    adhoc,
                    sc.absMainFilePath,
            ...
            model_dataset.svSet = Tools.restoreOrder(
                    model_dataset.svList,
                    ordered_list_for_all_vars,
                    model_dataset.svSet
            );
            model_dataset.asSet = Tools.restoreOrder(
                    model_dataset.asList,
                    ordered_list_for_all_vars,
                    model_dataset.asSet
            );
            ...
            if (redefined_global_file.size() > 0)
                LogUtils.errMsg("Global include files redefined: " + redefined_global_file);
            ...
            if (redefined_globals_as_locals.size() > 0)
                LogUtils.warningMsg("Global variables redefined as local: " + redefined_globals_as_locals);
...
//      cumulative_global_remove_redefined.remove_set(redefined_globals_as_locals); 
            ...
            try {
                cumulative_global_replace_redefined.replaceGlobalWithDuplicateLocal(
                        redefined_globals_as_locals,
                        model_dataset
                );
            ...
            checkUsedBeforeDefined(
                    new SimulationDataSet(model_dataset),
                    ordered_list_svar_alias_extern,
                    previous_global_svar_alias_extern_timeseries
            );

            /// check if goal dependents are defined
            // 1. get union of previous global vars and current model vars
            Set<String> allVars = new HashSet<String>();
            allVars.addAll(td.cumulative_global_complete.dvSet);
            allVars.addAll(model_dataset.dvSet);
            allVars.addAll(td.cumulative_global_complete.svSet);
            allVars.addAll(model_dataset.svSet);
            allVars.addAll(td.cumulative_global_complete.tsSet);
            allVars.addAll(model_dataset.tsSet);
            allVars.addAll(td.cumulative_global_complete.exSet);
            allVars.addAll(model_dataset.exSet);
            allVars.addAll(td.cumulative_global_complete.asSet);
            allVars.addAll(model_dataset.asSet);
            ...
            if (weightElement_undefined.size() > 0)
                LogUtils.errMsg("Weight var not found in dvar: " + weightElement_undefined);
        ...
        for (String key : model_dataset_map.keySet()) {
            ...
            for (String e : allVar) {
                if (e.length() > 99) LogUtils.errMsg("Variable name is longer than 99 chars: " + e);
            }
    ...
    private static Map<String, SimulationDataSet> getNewDataSet(
            Set<String> adhoc_incFileSet,
            Set<String> fileDataMap_wholeStudy_keySet
    ) throws RecognitionException, IOException {
        ...
        for (String f : adhoc_incFileSet) {
            ...
            Map<String, SimulationDataSet> each = FileParser.processNestedFileExceptFor(
                    f,
                    fileDataMap_wholeStudy_keySet
            );
    ...
    private static Map<String, SimulationDataSet> copyDataSetToThisModel(
            Set<String> incFileSet,
    ...
    private static ArrayList<String> getOrderedList(
    ...
    private static SimulationDataSet correctScopeAndPrioritize(
            String modelName,
            SimulationDataSet adhoc,
    ...
    private static void checkUsedBeforeDefined(
            SimulationDataSet ds,
            ArrayList<String> ordered_list_sv_as_ex,
            Set<String> previous_global_sv_as_ex_ts
    ) {
    ...
    private static void analyzeVarNeededFromCycle_subfunction(
            String varName,
            Set<String> neededVarInCycle,
            String fromWresl,
            StudyDataSet sd
    ) {
        ...
        for (String neededCycle : neededCycleVarMap.keySet()) {
            ...
            } else if (cycleNameConditionMap.get(neededCycle).length() > 0 && !cycleNameConditionMap.get(neededCycle)
                                                                                                    .equalsIgnoreCase(
                                                                                                            "always")) {
            ...
            for (String neededVar : neededVarSet) {
                ...
                if (!validVarNames.contains(neededVar)) {
                    ...
                    if (sd.getModelDataSetMap()
                          .get(neededCycle.toLowerCase()).tsList.contains(neededVar.toLowerCase())) {
