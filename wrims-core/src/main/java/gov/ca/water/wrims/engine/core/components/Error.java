...
import gov.ca.water.wrims.engine.core.wreslparser.elements.Tools;
...
public class Error {
    ...
    public static void addEvaluationError(String error) {
        ...
        if (ControlData.currEvalTypeIndex == 0) {
            ...
            LOGGER.atError().setMessage("# Error :" + msg).log();
        }
        if (ControlData.currEvalTypeIndex < 100)
            LOGGER.atError().setMessage(sourceLocation + " # Error :" + ControlData.currEvalName + ":" + error).log();
    }
    ...
    public static void addSolvingError(String error) {
        ...
        LOGGER.atError().setMessage("# Error :" + error).log();
    }
    ...
    public static void addInfeasibleHint(String goalName, String moreMsg) {
        ...
        LOGGER.atError().setMessage(sourceLocation + " " + goalName).log();
    }

    public static void addEngineError(String error) {
        error_engine.add("Engine Error: " + error);
        LOGGER.atError().setMessage("# Engine Error: " + error).log();
    }
    ...
    public static int getTotalError() {
        return Error.error_engine.size() + Error.error_evaluation.size() + Error.error_grammer.size() + Error.error_solving.size() + Error.error_config.size() + Error.error_initial.size() + Error.error_deviation.size();
    }
