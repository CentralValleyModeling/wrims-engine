...
public class DSSHDF5Converter {
    ...
    private static final ArrayList<String> svMonthlyKinds = new ArrayList<String>();
    ...
    private static final ArrayList<String> svDailyKinds = new ArrayList<String>();
    ...
    private static final ArrayList<String> initMonthlyKinds = new ArrayList<String>();
    ...
    private static final ArrayList<String> initDailyKinds = new ArrayList<String>();
    ...
    public void process() {
        convertSVFile();
        convertInitFile();
        LOGGER.atInfo().setMessage("Conversion done").log();
    }
    ...
    public void convertSVFileDssToHDF5() {
        ...
        LOGGER.atInfo().setMessage("Read Svar Dss File").log();
        readMonthlySVDssFile();
        readDailySVDssFile();
        LOGGER.atInfo().setMessage("Write Svar HDF5 File").log();
    ...
    public void convertInitFileDssToHDF5() {
        ...
        LOGGER.atInfo().setMessage("Read Initial Dss File").log();
        readMonthlyInitDssFile();
        readDailyInitDssFile();
        LOGGER.atInfo().setMessage("Write Initial HDF5 File").log();
    ...
    public void readMonthlySVDssFile() {
        ...
        CondensedReferenceCache cache = CondensedReferenceCacheAndRead.createCondensedCache(
                FilePaths.fullSvarFilePath,
                pathnameFilter
        );
    ...
    public static void closeSVHDF5DataStructure() {
        try {
            if (svGidDaily >= 0) H5.H5Gclose(svGidDaily);
            if (svGidMonthly >= 0) H5.H5Gclose(svGidMonthly);
            if (svGidData >= 0) H5.H5Gclose(svGidData);
            if (svGidInfo >= 0) H5.H5Gclose(svGidInfo);
            if (svGidIOData >= 0) H5.H5Gclose(svGidIOData);
            if (svGidPartF >= 0) H5.H5Gclose(svGidPartF);
            if (svGidPartA >= 0) H5.H5Gclose(svGidPartA);
            if (svFid >= 0) H5.H5Fclose(svFid);
    ...
    public void readMonthlyInitDssFile() {
        ...
        firstDateMonthly = new Date(
                ControlData.startYear - 1900,
                ControlData.startMonth - 1,
                TimeOperation.numberOfDays(ControlData.startMonth, ControlData.startYear)
        );
        for (String dp : cache.getAllPaths()) {
            TimeSeriesContainer tsc = cache.readFullRecord(dp);
            if (tsc != null) {
    ...
    public void readDailyInitDssFile() {
        ...
        if (dim > 0) {
            CondensedReferenceCache svarCache = CondensedReferenceCacheAndRead.createCondensedCache(
                    FilePaths.fullSvarFilePath,
                    pathnameFilter
            );
            for (String dp : cache.getAllPaths()) {
    ...
    public static void createInitHDF5DataStructure() {
        ...
        String[] monthlyDateStr = {TimeOperation.dssTimeEndDay(
                firstYear,
                firstMonth,
                TimeOperation.numberOfDays(firstMonth, firstYear)
        )};
        ...
        try {
            ...
            if (initGidInfo >= 0 && sidLookup >= 0) {
                ...
                if (didLookup >= 0) {
                    ...
                    tidCompoundTmp = H5.H5Tcreate(HDF5Constants.H5T_COMPOUND, 4);
                    H5.H5Tinsert(tidCompoundTmp, "Timestep", offset, tidStringTimestep);
                    HDF5Util.writeStringData(
                            didLookup,
                            tidCompoundTmp,
                            initLookupTimestep.toArray(new String[size]),
                            4
                    );
    ...
    public static void closeInitHDF5DataStructure() {
        try {
            if (initGidDaily >= 0) H5.H5Gclose(initGidDaily);
            if (initGidMonthly >= 0) H5.H5Gclose(initGidMonthly);
            if (initGidData >= 0) H5.H5Gclose(initGidData);
            if (initGidInfo >= 0) H5.H5Gclose(initGidInfo);
            if (initGidIOData >= 0) H5.H5Gclose(initGidIOData);
            if (initGidPartF >= 0) H5.H5Gclose(initGidPartF);
            if (initGidPartA >= 0) H5.H5Gclose(initGidPartA);
            if (initFid >= 0) H5.H5Fclose(initFid);
        } catch (HDF5LibraryException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
