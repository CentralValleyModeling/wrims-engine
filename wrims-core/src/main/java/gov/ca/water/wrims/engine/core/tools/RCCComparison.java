...
public class RCCComparison {
    ...
    private final ArrayList<Object> gNameArrayList = new ArrayList<Object>();
    private final ArrayList<Object> wNameArrayList = new ArrayList<Object>();
    private final ArrayList<Object> dvarArrayList = new ArrayList<Object>();
    private final Map<String, WeightElement> weightMap;
    private final Map<String, Dvar> dvarMap;
    ...
    public RCCComparison() {
        ...
        try {
            ...
            for (int i = 0; i < wNameArrayList.size(); i++) {
                ...
                out1.write(weightMap.get(wNameArrayList.get(i))
                                    .getValue() + "*" + wNameArrayList.get(i) + " is not in WRIMS1.\n");
            }
            out1.close();
        } catch (Exception e) {
            e.printStackTrace();
            LOGGER.atInfo().setMessage(gName).log();
        }
    ...
    public void compareObject(String strLine) throws IOException {
        ...
        strLine = strLine.substring(2).toLowerCase();
    ...
    public void compareConstraint(String strLine) throws IOException {
        ...
        if (constraintMap.containsKey(gName)) {
            ...
            for (int i = 0; i < multiplierArrayList.size(); i++) {
                isDifferent = true;
                outLine = outLine + "(no|" + multiplier.get(multiplierArrayList.get(i))
                                                       .getData() + ")" + multiplierArrayList.get(i) + ";";
            }
