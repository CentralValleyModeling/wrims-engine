...
//	}	
    ...
    // these dvars are from slack and surplus of weight group deviation penalty
    public static boolean checkDeviationSlackSurplus(
            Map<String, Double> deviationSS_toleranceMap,
    ...
    public static int checkVarRedefined(ModelTemp m, StudyTemp st) {
        ...
        if (itemDup.size() > 0) {
            ...
            for (String s : itemDup) {
                String item = "Item";
                int line = 0;
                if (m.dvList.contains(s)) {
                    line = m.dvMap.get(s).line;
                    item = "dvar";
                } else if (m.svList.contains(s)) {
                    line = m.svMap.get(s).line;
                    item = "svar";
                } else if (m.tsList.contains(s)) {
                    line = m.tsMap.get(s).line;
                    item = "timeseries";
                } else if (m.glList.contains(s)) {
                    line = m.glMap.get(s).line;
                    item = "goal";
                } else if (m.asList.contains(s)) {
                    line = m.asMap.get(s).line;
                    item = "alias";
                } else if (m.exList.contains(s)) {
                    line = m.exMap.get(s).line;
                    item = "external";
                }
