...
public class DetectorWorkflow {
    ...
    protected boolean validateOriginalModel() {
        ...
        // log original model
        if (DetectorParam.lpsLogging) MPModelUtils.toLpSolve(
                originalModel,
                DetectorParam.lpsDir,
                DetectorParam.lpsFileNamePrepend + "_original.lps"
        );
        // solve original model
        LOGGER.atInfo().setMessage("solve original model...").log();
        ...
        if (originalModelReturnCode != 0) {
            ...
            LOGGER.atInfo().setMessage("solving success for original model!").log();
...
//	}
    ...
    protected boolean validateBaseModel(double obj_constraint_relax_ratio) {
        ...
        if (!isOriginalValidated) return false;
        ...
        baseModel.createConstraint(
                "OBJ_CONSTRAINT",
                originalModel.objFunction,
                originalModel.objValue * (1.0 - obj_constraint_relax_ratio),
                Param.inf
        );
        ...
        LOGGER.atInfo().setMessage("solve base model...").log();
        ...
        if (baseModelReturnCode != 0) {
            ...
            LOGGER.atInfo().setMessage("solving success for base model!").log();
        }
    ...
    public int findAltSolutions(ArrayList<String> searchVarList) {
        ...
        if (!(isBaseValidated && isOriginalValidated)) return DetectorParam.modelNotValid;

        // use all vars in solution to search if null
        if (searchVarList == null) searchVarList = new ArrayList<String>(baseModel.solution.keySet());
        ...
        if (DetectorParam.searchLevel != 2) return DetectorParam.otherErrors; // not integrated
        ...
        AltSolutionFinder altFinder1 = new AltSolutionFinder(
                "group1",
                                                             baseModel,
                                                             g1searchVars,
                                                             searchObjSign1,
        ...
        if (group1Solutions.size() == 0) {
            LOGGER.atInfo().setMessage(" no alt solution found in group1").log();
        } else {
            LOGGER.atInfo().setMessage(" alt solution found in group1").log();
        ...
        LOGGER.atError().setMessage("begin group2").log();
        ...
        AltSolutionFinder altFinder2 = new AltSolutionFinder(
                "group2",
                                                             baseModel,
                                                             g2searchVars,
                                                             searchObjSign2,
        ...
        if (group2Solutions.size() == 0) {
            LOGGER.atInfo().setMessage(" no alt solution found in group2").log();
        } else {
            varRange_group2 = Misc.findVarsRange(group2Solutions);
        }
        if (varRange_group2.size() > 0) {
            LOGGER.atInfo().setMessage(" alt solution found in group2").log();
        ...
        if (varRange_allGroups.size() > 0) {
            LOGGER.atInfo().setMessage("update vars range...").log();
        ...
        varsRange_output = Misc.filterVarsRange(
                varRange_all,
                DetectorParam.nonunique_min_abs_diff,
                DetectorParam.nonunique_min_ratio_diff
        );
