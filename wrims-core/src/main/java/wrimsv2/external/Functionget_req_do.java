...
//		Gavg = (-LOG(arg2) / galpha)**(1.0 / n);
        ...
        if (G_o > Gavg) {
            G99avg = QNEW + (gbeta / gmdT) * (float) Math.log10((1.0f + (QNEW / G_o - 1.0f) * (float) Math.exp(-QNEW * gmdT / gbeta)) / (QNEW / G_o));
        ...
        if (Qreq < 0.1) {
            ...
            while (diff > gmtol) {
                ...
                YY = (QOLD * (float) Math.exp(-gmdT * (QOLD - Gavg) / gbeta) / G_o - (QOLD / G_o - 1.0f) * (float) Math.exp(
                        -gmdT * QOLD / gbeta) - 1.0f);
                DYDQ = (1.0f / G_o - QOLD * gmdT / (G_o * gbeta)) * (float) Math.exp(-gmdT * (QOLD - Gavg) / gbeta) - (1.0f / G_o - QOLD * gmdT / (G_o * gbeta) + gmdT / gbeta) * (float) Math.exp(
                        -gmdT * QOLD / gbeta);
